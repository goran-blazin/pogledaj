...
function getSeedData(): SeedDemoData {
  ...
  const generateProjections = (cinema: Cinema, movie: Movie):MovieProjection[] => {
    ...
    const days = _.shuffle([ ...Array(14).keys() ]).slice(0, 6).map(dayOffset => {
      // for each day return 4 random times between 9 and 23
      return _.shuffle([ ...Array(14).keys() ]).slice(0, 4).map(hourOffset => {
        return today.plus({
          day: dayOffset,
          hour: hourOffset
        })
    ...
    return _.flatten(days).sort((a: DateTime, b: DateTime) => {
