...
function CinemaSingleWrapper() {
  ...
  const [projectionsGroupedPerDateAndMovie, setProjectionsGroupedPerDateAndMovie] = useState<
    ProjectionsGroupedPerDateAndMovieType | undefined
  >(undefined);
  const selectedDate = searchParams.get('selectedDate');

  useEffect(() => {
    if (cinemaId !== undefined) {
      Promise.all([CinemasService.findById(cinemaId), MovieProjectionsService.findAllByCinema(cinemaId)])
        .then(([cinema, movieProjections]) => {
          setCinema(cinema);
          if (cinema) {
            setProjectionsGroupedPerDateAndMovie(
              movieProjections.reduce((carry: ProjectionsGroupedPerDateAndMovieType, mp) => {
          ...
          {cinema ? (
            <div>
              <div>
                ...
                <p>Telefon: {cinema.phone.length ? cinema.phone.join(', ') : <template>Nema telefona</template>}</p>
              </div>
              <div>
