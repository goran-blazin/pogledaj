...
function ProjectionsGroupedPerDateAndMovie({groupedMovieProjections, date}:
  {groupedMovieProjections: ProjectionsGroupedPerMoviesType, date: string}
) {
  return (
    <div>
              ...
              {movieProjections.map(mp => {
                return (
                  <span key={mp.movieProjectionId}>
                    <Link to={`/movie-projections/${mp.movieProjectionId}`}>
                      {mp.time}
                    </Link>
                    &nbsp;
                  </span>
